...
static u32 x2_bifsd_alloc_mem(size_t size)
{
#ifdef CONFIG_SYS_NONCACHED_MEMORY
	return (u32)noncached_alloc(size, X2_BIFSD_ALIGN);
#else
	return (u32)memalign(X2_BIFSD_ALIGN, size);
#endif
}
...
void dev_info_init(void)
{
    ...
    mmc_info->rw_buf = (u32)x2_bifsd_alloc_mem(256*1024);
...
static int x2_bifsd_probe(struct udevice *dev)
{
	x2_bifsd_init(dev);
	return 0;
}

static const struct udevice_id x2_bifsd_ids[] = {
	{ .compatible = "hobot,bifsd" },
	{ }
};

U_BOOT_DRIVER(x2_bifsd_drv) = {
	.name		= "x2_bifsd_drv",
	.of_match	= x2_bifsd_ids,
	.probe		= x2_bifsd_probe,
