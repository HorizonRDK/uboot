...
#ifndef HOBOT_IAR_DRIVER
#define HOBOT_IAR_DRIVER
...
struct xj3_iar_priv
{
        struct clk iar_pix_clk;
        unsigned long iar_bt1120_pix_clk;
        int output_mode;
        struct disp_timing display_timing;
        channel_base_cfg_t iar_channel_base_cfg;
        output_cfg_t iar_output_cfg;
};

#ifdef CONFIG_XJ3_DISPLAY_DEBUG
#define xj3_display_debug(fmt, args...)      \
        do                                   \
        {                                    \
                if (1)                       \
                        printf(fmt, ##args); \
